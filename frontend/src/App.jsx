...
import { 
  Layout, Download, Shield, AlertCircle, Zap, TrendingUp, Copy, 
  Upload, FileText, CheckCircle, ChevronRight, Search 
} from "lucide-react";
...
export default function App() {
  ...
  const [resumeFile, setResumeFile] = useState(null); // New state for file
  const [analysisData, setAnalysisData] = useState(null); // Results start as null
        ...
        <div className="w-full max-w-[1280px] flex justify-between items-center">
          ...
          {analysisData && (
             <div className="hidden md:block text-sm text-slate-400">
               Target: <span className="text-white font-medium">{analysisData.roles[0].role}</span>
             </div>
          )}
        </div>
      </header>
        ...
        <div className="max-w-[1280px] mx-auto space-y-8">
          ...
          <section className="space-y-6">
            <div className="text-center space-y-2 mb-8">
              ...
              <p className="text-slate-400">Paste your job description and resume below to get an AI-powered gap analysis.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="border-2 border-dashed border-slate-600 rounded-xl p-8 w-full text-center hover:border-blue-500 transition-colors cursor-pointer relative">
                  <input 
                      type="file" 
                      accept=".pdf"
          ...
          {analysisData && (
            <div ref={resultsRef} className="space-y-6 pt-12 border-t border-white/5 animate-in fade-in slide-in-from-bottom-10 duration-700">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <GlassCard className="relative overflow-hidden group">
                  ...
                  <div className="relative z-10">
                    ...
                    <p className="text-xs text-slate-400 mt-4">Target Role: {analysisData.roles[0]?.role}</p>
                  </div>
                </GlassCard>
                ...
                <GlassCard className="md:col-span-2">
                  <div className="flex justify-between items-start mb-4">
                     ...
                     <Badge color="red">{analysisData.missing_skills.length} Missing</Badge>
                  </div>
              ...
              {/* --- 3-COLUMN DASHBOARD --- */}
              <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr_360px] gap-6 items-start">
                
                {/* LEFT: Stats */}
                <div className="space-y-6">
                   <GlassCard title="Document Stats">
                     <div className="space-y-4 text-sm">
                       <div className="flex justify-between border-b border-white/5 pb-2">
                         <span className="text-slate-400">Resume Words</span>
                         <span className="text-white font-mono">{analysisData.meta.resume_word_count}</span>
                       </div>
                       <div className="flex justify-between border-b border-white/5 pb-2">
                         <span className="text-slate-400">JD Words</span>
                         <span className="text-white font-mono">{analysisData.meta.jd_word_count}</span>
                       </div>
                       <div className="flex justify-between items-center">
                         <span className="text-slate-400">Format Score</span>
                         <span className={`font-bold ${analysisData.scores.format_score < 70 ? 'text-amber-400' : 'text-green-400'}`}>
                            {analysisData.scores.format_score}
                         </span>
                       </div>
                     </div>
                   </GlassCard>
                    ...
                    <div className="space-y-4">
                      {analysisData.roles.map((role, idx) => (
                        <div key={idx} className={`p-4 rounded-xl border transition-all ${idx === 0 ? 'bg-blue-500/10 border-blue-500/30' : 'bg-slate-800/50 border-white/5'}`}>
                          ...
                          <p className="text-xs text-slate-400">{role.reason.substring(0, 120)}...</p>
                        </div>
                      ))}
                    </div>
                  </GlassCard>

                  <GlassCard title="ATS Keyword Boosters">
                    <div className="space-y-4">
                      {analysisData.booster_suggestions.map((boost, i) => (
                        <div key={i} className="bg-slate-800/40 p-4 rounded-lg border border-white/5 hover:border-blue-500/30 transition-colors group">
                          ...
                          <p className="text-sm text-slate-300 font-mono bg-black/20 p-3 rounded border border-white/5 leading-relaxed">
                            {boost.snippet}
                          </p>
                        </div>
                      ))}
                     ...
                     <div className="space-y-3">
                       {analysisData.future_trends?.map((trend, i) => (
                         <div key={i} className="p-3 bg-slate-800/50 rounded-lg border border-white/5 flex items-start gap-3">
                           ...
                           <div>
                             <h5 className="text-sm font-medium text-emerald-100">{trend.name}</h5>
